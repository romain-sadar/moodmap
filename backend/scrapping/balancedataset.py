...
df_reviews["review"] = (
    df_reviews["review"].astype(str).str.strip()
)  # Supprime les espaces vides
df_reviews.replace(
    {"": None, "nan": None}, inplace=True
)  # Remplace les chaînes vides et 'nan' par None
...
places_still_without_reviews = df_places[
    ~df_places["label"].isin(places_with_reviews_final)
]
...
default_reviews = {
    5: [
        ...
        "Perfect service and atmosphere!",
    ],
    4: [
        "Great experience, but a few minor issues.",
    ...
    3: [
        ...
        "A neutral visit, nothing bad or great.",
    ],
    2: [
...
for _, row in places_still_without_reviews.iterrows():
    for _ in range(random.randint(5, 10)):  # 5 à 10 avis par lieu
        rating = random.choices(
            list(rating_distribution.keys()), weights=rating_distribution.values(), k=1
        )[0]

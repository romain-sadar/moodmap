...
if not all(mood in existing_moods for mood in moods_needed):
    ...
    for mood in moods_needed:
        if mood not in existing_moods:
            sample_reviews = df_reviews[df_reviews["mood"] == mood].sample(
                n=2, replace=True
            )  # Prend 2 exemples
            df_mood_per_place = pd.concat(
                [df_mood_per_place, sample_reviews[["place", "mood"]]],
                ignore_index=True,
            )

# Sauvegarder le fichier final
df_mood_per_place.to_csv(
    "restaurants_moods_balanced.csv", index=False, encoding="utf-8"
)
print(
